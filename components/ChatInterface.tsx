...
export function ChatInterface({
  ...
  return (
    <div className="w-full h-full flex flex-col">
      {/* ── Message History — fills available space ── */}
      <div className="flex-1 min-h-0 relative">
        {displayMessages.length > 0 ? (
          <>
            {/* Top fade gradient — messages dissolve upward */}
            <div className="absolute top-0 left-0 right-0 h-10 z-10 pointer-events-none" style={{
              background: 'linear-gradient(to bottom, rgba(12, 10, 9, 0.55) 0%, transparent 100%)',
            }} />

            <div
              ref={scrollContainerRef}
              className="h-full overflow-y-auto custom-scrollbar pr-1 space-y-2 pt-8 pb-1"
            >
              {displayMessages.map((message, index) => (
                ...
                >
                  {message.role === 'assistant' && (
                    <div className="w-4.5 h-4.5 rounded flex-shrink-0 mr-1.5 mt-0.5 flex items-center justify-center" style={{
                      background: 'var(--copper-glow)',
                      border: '1px solid var(--border-copper)',
                      width: '18px', height: '18px',
                    }}>
                      <svg className="w-2 h-2" style={{ color: 'var(--copper)' }} fill="currentColor" viewBox="0 0 24 24">
                        <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
                      </svg>
                    </div>
                  )}
                  <div
                    className="max-w-[82%] px-2.5 py-1.5 rounded-lg text-xs"
                    style={{
              ...
              {isLoading && (
                <div className="flex justify-start animate-slide-up" style={{ opacity: 0, animationFillMode: 'forwards' }}>
                  <div className="rounded flex-shrink-0 mr-1.5 mt-0.5 flex items-center justify-center" style={{
                    background: 'var(--copper-glow)',
                    border: '1px solid var(--border-copper)',
                    width: '18px', height: '18px',
                  }}>
                  ...
                  <div className="px-2.5 py-2 rounded-lg flex items-center gap-1.5" style={{
                    ...
                    <span className="text-[11px]" style={{ color: 'var(--text-muted)' }}>Thinking...</span>
                  </div>
                </div>
              )}
          ...
          <div className="h-full flex items-center justify-center text-center">
            <div className="space-y-2.5 animate-fade-in">
              <div className="w-10 h-10 mx-auto rounded-lg flex items-center justify-center" style={{
      ...
      <div className="flex-shrink-0 pt-2.5 mt-1" style={{ borderTop: '1px solid var(--border-subtle)' }}>
        <div className="flex gap-1.5">
          ...
          <button
            ...
            className="btn-copper w-9 h-9 !p-0 flex items-center justify-center flex-shrink-0"
          >
            {isLoading ? (
              <div className="w-3.5 h-3.5 rounded-full animate-spin" style={{
        ...
        {voiceRecording?.isRecording && (
          <div className="flex items-center gap-2 mt-1.5 animate-fade-in">
            ...
            <span className="text-[11px]" style={{ color: 'var(--danger)' }}>Recording...</span>
          </div>
        )}
      </div>
    </div>
